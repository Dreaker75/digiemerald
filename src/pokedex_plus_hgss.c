...
static void Task_ExitCaughtMonPage(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        lzPaletteData = GetMonSpritePalFromSpeciesAndGender(species, gender);
...
static void Task_LoadStatsScreen(u8 taskId)
{
    ...
    case 5:
        if (gTasks[taskId].data[1] == 0)
        {
            ...
            gTasks[taskId].data[6] = CreateMonIcon(NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum), SpriteCB_MonIcon, 18, 31, 4, 0, 0); //Create pokemon sprite
...
static void Task_LoadEvolutionScreen(u8 taskId)
{
    ...
    case 3:
        if (gTasks[taskId].data[1] == 0)
        {
            ...
            gTasks[taskId].data[4] = CreateMonIcon(NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum), SpriteCB_MonIcon, 18 + 32*sPokedexView->numPreEvolutions, 31, 4, 0, 0); //Create pokemon sprite
...
static void HandleTargetSpeciesPrint(u8 taskId, u16 targetSpecies, u16 previousTargetSpecies, u8 base_x, u8 base_y, u8 base_y_offset, u8 base_i, bool8 isEevee)
{
    ...
    if (base_i < iterations)
    {
        LoadMonIconPalette(targetSpecies); //Loads pallete for current mon
            if (isEevee)
                gTasks[taskId].data[4+base_i] = CreateMonIcon(targetSpecies, SpriteCB_MonIcon, 45 + 26*base_i, 31, 4, 0, 0); //Create pokemon sprite
            else
...
static void HandlePreEvolutionSpeciesPrint(u8 taskId, u16 preSpecies, u16 species, u8 base_x, u8 base_y, u8 base_y_offset, u8 base_i)
{
    ...
    if (base_i < 3)
    {
        ...
        gTasks[taskId].data[4+base_i] = CreateMonIcon(preSpecies, SpriteCB_MonIcon, 18 + 32*base_i, 31, 4, 0, 0); //Create pokemon sprite
        gSprites[gTasks[taskId].data[4+base_i]].oam.priority = 0;
    }
}
...
static void Task_LoadFormsScreen(u8 taskId)
{
    ...
    case 3:
        if (gTasks[taskId].data[1] == 0)
        {
            ...
            FreeMonIconPalettes(); //Free space for new pallete
            LoadMonIconPalette(NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum)); //Loads pallete for current mon
            gTasks[taskId].data[4] = CreateMonIcon(NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum), SpriteCB_MonIcon, 18, 31, 4, 0, 0); //Create pokemon sprite
            gSprites[gTasks[taskId].data[4]].oam.priority = 0;
        }
        EvoFormsPage_PrintNavigationButtons(); // Navigation buttons
...
static void PrintForms(u8 taskId, u16 species)
{
    ...
    for (i=0; i < 30; i++)
    {
        ...
        else
        {
            ...
            if (times < 7)
                gTasks[taskId].data[4+times] = CreateMonIcon(speciesForm, SpriteCB_MonIcon, 52 + 34*(times-1), 31, 4, 0, 0); //Create pokemon sprite
            else if (times < 14)
                gTasks[taskId].data[4+times] = CreateMonIcon(speciesForm, SpriteCB_MonIcon, 18 + 34*(times-7), 70 - y_offset_icons, 4, 0, 0); //Create pokemon sprite
