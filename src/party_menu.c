...
static void CreatePartyMonIconSpriteParameterized(u16, u8, u8, struct PartyMenuBox *, u8);
...
static void CreatePartyMonSprites(u8 slot)
{
    ...
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_MULTI_SHOWCASE && slot >= MULTI_PARTY_SIZE)
    {
        ...
        if (gMultiPartnerParty[actualSlot].species != SPECIES_NONE)
        {
            CreatePartyMonIconSpriteParameterized(gMultiPartnerParty[actualSlot].species, gMultiPartnerParty[actualSlot].form, gMultiPartnerParty[actualSlot].gender, &sPartyMenuBoxes[slot], 0);
...
static void CreatePartyMonIconSprite(struct Pokemon *mon, struct PartyMenuBox *menuBox, u32 slot)
{
    ...
    CreatePartyMonIconSpriteParameterized(species2, GetMonData(mon, MON_DATA_FORM), GetMonData(mon, MON_DATA_GENDER), menuBox, 1);
    UpdatePartyMonHPBar(menuBox->monSpriteId, mon);
}

static void CreatePartyMonIconSpriteParameterized(u16 species, u8 form, u8 gender, struct PartyMenuBox *menuBox, u8 priority)
{
    if (species != SPECIES_NONE)
    {
        menuBox->monSpriteId = CreateMonIcon(species, SpriteCB_MonIcon, menuBox->spriteCoords[0], menuBox->spriteCoords[1], 4, form, gender);
...
static void Task_TryItemUseFormChange(u8 taskId)
{
    ...
    case 3:
        ...
        if (gTasks[taskId].tAnimWait == 0)
        {
            ...
            CreatePartyMonIconSpriteParameterized(targetSpecies, GetMonData(mon, MON_DATA_FORM, NULL), GetMonData(mon, MON_DATA_GENDER, NULL), &sPartyMenuBoxes[gPartyMenu.slotId], 1);
            icon->oam.mosaic = TRUE;
            icon->data[0] = 10;
            icon->data[1] = 1;
            icon->data[2] = taskId;
