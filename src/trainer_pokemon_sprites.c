...
static bool16 DecompressPic(u16 species, u8 form, u8 gender, bool8 isFrontPic, u8 *dest, bool8 isTrainer)
{
    if (!isTrainer)
    {
...
static void LoadPicPaletteByTagOrSlot(u16 species, u8 gender, u8 paletteSlot, u16 paletteTag, bool8 isTrainer)
{
    if (!isTrainer)
    {
        if (paletteTag == TAG_NONE)
        {
            ...
            LoadCompressedPalette(GetMonSpritePalFromSpeciesAndGender(species, gender), OBJ_PLTT_ID(paletteSlot), PLTT_SIZE_4BPP);
        }
        else
        {
...
static void LoadPicPaletteBySlot(u16 species, u8 gender, u8 paletteSlot, bool8 isTrainer)
{
    if (!isTrainer)
        LoadCompressedPalette(GetMonSpritePalFromSpeciesAndGender(species, gender), PLTT_ID(paletteSlot), PLTT_SIZE_4BPP);
    else
...
static u16 CreatePicSprite(u16 species, u8 form, u8 gender, bool8 isFrontPic, s16 x, s16 y, u8 paletteSlot, u16 paletteTag, bool8 isTrainer)
{
    u8 i;
    u8 *framePics;
    struct SpriteFrameImage *images;
    ...
    if (DecompressPic(species, form, gender, isFrontPic, framePics, isTrainer))
    {
        // debug trap?
        return 0xFFFF;
    }
    ...
    LoadPicPaletteByTagOrSlot(species, gender, paletteSlot, paletteTag, isTrainer);
...
u16 CreateMonPicSprite_Affine(u16 species, u8 form, u8 gender, u8 flags, s16 x, s16 y, u8 paletteSlot, u16 paletteTag)
{
    u8 *framePics;
    struct SpriteFrameImage *images;
    int j;
    ...
    if (DecompressPic(species, form, gender, flags, framePics, FALSE))
    {
        // debug trap?
        return 0xFFFF;
    }
    ...
    sCreatingSpriteTemplate.callback = DummyPicSpriteCallback;
    LoadPicPaletteByTagOrSlot(species, gender, paletteSlot, paletteTag, FALSE);
    spriteId = CreateSprite(&sCreatingSpriteTemplate, x, y, 0);
    if (paletteTag == TAG_NONE)
        gSprites[spriteId].oam.paletteNum = paletteSlot;
    sSpritePics[i].frames = framePics;
...
static u16 LoadPicSpriteInWindow(u16 species, u8 form, u8 gender, bool8 isFrontPic, u8 paletteSlot, u8 windowId, bool8 isTrainer)
{
    if (DecompressPic(species, form, gender, isFrontPic, (u8 *)GetWindowAttribute(windowId, WINDOW_TILE_DATA), FALSE))
        return 0xFFFF;

    LoadPicPaletteBySlot(species, gender, paletteSlot, isTrainer);
    return 0;
}

static u16 CreateTrainerCardSprite(u16 species, u8 form, u8 gender, bool8 isFrontPic, u16 destX, u16 destY, u8 paletteSlot, u8 windowId, bool8 isTrainer)
{
    u8 *framePics;

    framePics = Alloc(TRAINER_PIC_SIZE * MAX_TRAINER_PIC_FRAMES);
    if (framePics && !DecompressPic(species, form, gender, isFrontPic, framePics, isTrainer))
    {
...
u16 CreateMonPicSprite(u16 species, u8 form, u8 gender, bool8 isFrontPic, s16 x, s16 y, u8 paletteSlot, u16 paletteTag)
{
    return CreatePicSprite(species, form, gender, isFrontPic, x, y, paletteSlot, paletteTag, FALSE);
}
...
static u16 UNUSED LoadMonPicInWindow(u16 species, u8 form, u8 gender, bool8 isFrontPic, u8 paletteSlot, u8 windowId)
{
    return LoadPicSpriteInWindow(species, form, gender, isFrontPic, paletteSlot, windowId, FALSE);
}

// Unused, FRLG only
u16 CreateTrainerCardMonIconSprite(u16 species, u8 form, u8 gender, bool8 isFrontPic, u16 destX, u16 destY, u8 paletteSlot, u8 windowId)
{
    return CreateTrainerCardSprite(species, form, gender, isFrontPic, destX, destY, paletteSlot, windowId, FALSE);
}
...
{
    return LoadPicSpriteInWindow(species, 0, FEMALE, isFrontPic, paletteSlot, windowId, TRUE);
}

u16 CreateTrainerCardTrainerPicSprite(u16 species, bool8 isFrontPic, u16 destX, u16 destY, u8 paletteSlot, u8 windowId)
{
    return CreateTrainerCardSprite(species, 0, FEMALE, isFrontPic, destX, destY, paletteSlot, windowId, TRUE);
}
